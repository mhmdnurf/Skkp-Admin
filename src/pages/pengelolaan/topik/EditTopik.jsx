...
export const EditTopik = () => {
  ...
  const [dataNamaTopik, setDataNamaTopik] = useState("");
  const [user, loading] = useAuthState(auth);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, "topikPenelitian", itemId);
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const data = docSnap.data();
          setNamaTopik(data.namaTopik);
          setDataNamaTopik(data.namaTopik);
        } else {
  ...
  const handleFormEdit = async (e) => {
    ...
    if (!namaTopik) {
      setError("Topik Penelitian harus diisi");
      return;
    }
    ...
    try {
      ...
      if (namaTopik !== dataNamaTopik) {
        const querySnapshot = await getDocs(
          query(
            collection(db, "topikPenelitian"),
            where("namaTopik", "==", namaTopik)
          )
        );

        if (!querySnapshot.empty) {
          Swal.fire("Error", "Topik Penelitian sudah dibuat", "error");
          setError("Topik Penelitian sudah ada");
      ...
      await updateDoc(docRef, { namaTopik: namaTopik });
