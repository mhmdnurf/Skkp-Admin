...
const EditPersyaratan = () => {
  ...
  React.useEffect(() => {
    const fetchPersyaratan = async () => {
      const docRef = doc(db, "persyaratan", itemId);
      const docSnap = await getDoc(docRef);

      if (docSnap.exists()) {
        const data = docSnap.data();
        setFormData({
          jenisPersyaratan: data.jenisPersyaratan,
          berkasPersyaratan: "",
          berkasList: data.berkasPersyaratan,
        });
      } else {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        const persyaratanRef = doc(db, "persyaratan", itemId);
        await updateDoc(persyaratanRef, {
          jenisPersyaratan,
          berkasPersyaratan: berkasList,
        });

        Swal.fire("Success", "Persyaratan berhasil diupdate!", "success").then(
          () => {
      ...
      console.error("Error update persyaratan: ", error);
...
export default EditPersyaratan;
