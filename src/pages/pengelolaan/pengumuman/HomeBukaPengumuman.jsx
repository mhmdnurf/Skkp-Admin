...
export const HomePengumuman = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      query(collection(db, "pengumuman")),
      (snapshot) => {
        ...
        const filteredData = fetchedData.filter(
          (item) =>
            ...
            item.pesan.toLowerCase().includes(searchText.toLowerCase())
        );
              ...
              <div className="flex justify-between mt-16">
                <div className="flex items-center ml-4 ">
                  <Link
                    ...
                    className="text-center px-4 py-2 border w-[250px] rounded-md drop-shadow-lg bg-slate-600 text-white font-bold hover:bg-slate-700"
                  >
                    Tambah Pengumuman
                  </Link>
                </div>
                ...
                <table className="w-full bg-white rounded-t-lg text-slate-700 drop-shadow-md">
                  <thead className="shadow-sm font-extralight text-sm">
                    <tr>
                      ...
                      <th className="p-2 px-6">Pesan</th>
                  ...
                  <tbody className="rounded-b-md text-sm">
                    {currentItems.map((item, index) => (
                      ...
                      >
                        ...
                        <td className="text-center">{item.pesan}</td>
